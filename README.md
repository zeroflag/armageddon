 ...
 * `forth.com`
 ...
 The `make.bat` script builds the project (by redirecting `forth.asm` to `debug.com`'s stdin), runs `forth.com` and loads `core.forth`). You can start `forth.com` afterwards, but by default the `core.forth` file is not loaded.
